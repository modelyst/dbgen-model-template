...
from ..extracts.measurement_extract import \
    MeasurementExtract
...
def add_temperature_generator(model: Model) -> None:
    with model:
        with Generator(name="temperature"):
            filename, contents = MeasurementExtract(
                data_dir=join(DATA_DIR, "measurements")
            ).results()
            first_name, last_name, ordering, temperature = parse_measurements(
                filename, contents
            ).results()
